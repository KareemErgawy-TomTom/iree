...
typedef struct iree_hal_command_list_t {
...
static void iree_hal_command_list_initialize(
    iree_arena_block_pool_t* block_pool,
    iree_hal_command_list_t* out_command_list) {
  iree_arena_initialize(block_pool, &out_command_list->arena);
  out_command_list->head = NULL;
  out_command_list->tail = NULL;
}

// Resets the command list and returns all arena blocks back to the block pool.
// Upon return the command list is ready for recording.
static void iree_hal_command_list_reset(iree_hal_command_list_t* command_list) {
  ...
  // the pool.
  iree_arena_reset(&command_list->arena);
  command_list->head = NULL;
  command_list->tail = NULL;
}

// Deinitializes the command list, preparing for destruction.
static void iree_hal_command_list_deinitialize(
    iree_hal_command_list_t* command_list) {
  iree_hal_command_list_reset(command_list);
}
...
static iree_status_t iree_hal_command_list_append_command(
    iree_hal_command_list_t* command_list, iree_hal_cmd_type_t command_type,
    iree_host_size_t command_size, void** out_cmd) {
  iree_hal_cmd_header_t* header = NULL;
  IREE_RETURN_IF_ERROR(
      iree_arena_allocate(&command_list->arena, command_size, (void**)&header));
  header->next = NULL;
  header->type = command_type;
  if (!command_list->head) {
    command_list->head = header;
  } else if (command_list->tail) {
    command_list->tail->next = header;
  }
  command_list->tail = header;
  *out_cmd = header;
  return iree_ok_status();
}

// Clones a source buffer and returns the pointer into the arena.
static iree_status_t iree_hal_command_list_clone_data(
    iree_hal_command_list_t* command_list, const void* source_data,
    iree_host_size_t data_length, void** out_target_data) {
  void* target_data = NULL;
  IREE_RETURN_IF_ERROR(
      iree_arena_allocate(&command_list->arena, data_length, &target_data));
...
IREE_API_EXPORT iree_status_t iree_hal_deferred_command_buffer_create(
  ...
  *out_command_buffer = NULL;
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_hal_command_list_initialize(block_pool, &command_buffer->command_list);
  }
...
static void iree_hal_deferred_command_buffer_destroy(
  ...
  iree_hal_command_list_deinitialize(&command_buffer->command_list);
...
static iree_status_t iree_hal_deferred_command_buffer_begin(
  ...
  iree_hal_deferred_command_buffer_t* command_buffer =
      iree_hal_deferred_command_buffer_cast(base_command_buffer);
  iree_hal_command_list_reset(&command_buffer->command_list);
  return iree_ok_status();
}
...
static iree_status_t iree_hal_deferred_command_buffer_execution_barrier(
    ...
    const iree_hal_buffer_barrier_t* buffer_barriers) {
  iree_hal_command_list_t* command_list =
      &iree_hal_deferred_command_buffer_cast(base_command_buffer)->command_list;
  iree_hal_cmd_execution_barrier_t* cmd = NULL;
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_EXECUTION_BARRIER, sizeof(*cmd), &cmd));
  ...
  if (memory_barrier_count > 0) {
    IREE_RETURN_IF_ERROR(iree_hal_command_list_clone_data(
        command_list, memory_barriers,
        sizeof(memory_barriers[0]) * memory_barrier_count,
        (void**)&cmd->memory_barriers));
  }
  if (buffer_barrier_count > 0) {
    IREE_RETURN_IF_ERROR(iree_hal_command_list_clone_data(
        command_list, buffer_barriers,
...
static iree_status_t iree_hal_deferred_command_buffer_signal_event(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_SIGNAL_EVENT, sizeof(*cmd), &cmd));
  cmd->event = event;
  cmd->source_stage_mask = source_stage_mask;
  return iree_ok_status();
}
...
static iree_status_t iree_hal_deferred_command_buffer_reset_event(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_RESET_EVENT, sizeof(*cmd), &cmd));
  cmd->event = event;
  cmd->source_stage_mask = source_stage_mask;
  return iree_ok_status();
}
...
static iree_status_t iree_hal_deferred_command_buffer_wait_events(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_WAIT_EVENTS,
      sizeof(*cmd) + sizeof(cmd->events[0]) * event_count, &cmd));
...
static iree_status_t iree_hal_deferred_command_buffer_discard_buffer(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_DISCARD_BUFFER, sizeof(*cmd), &cmd));
  cmd->buffer = buffer;
  return iree_ok_status();
}
...
static iree_status_t iree_hal_deferred_command_buffer_fill_buffer(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_FILL_BUFFER, sizeof(*cmd), &cmd));
...
static iree_status_t iree_hal_deferred_command_buffer_apply_fill_buffer(
  ...
  return iree_hal_command_buffer_fill_buffer(
      ...
      cmd->length, &cmd->pattern, cmd->pattern_length);
}
...
static iree_status_t iree_hal_deferred_command_buffer_update_buffer(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_UPDATE_BUFFER,
      sizeof(*cmd) + sizeof(cmd->source_buffer[0]) * length, &cmd));
  cmd->target_buffer = target_buffer;
  cmd->target_offset = target_offset;
  cmd->length = length;
...
static iree_status_t iree_hal_deferred_command_buffer_copy_buffer(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_COPY_BUFFER, sizeof(*cmd), &cmd));
...
static iree_status_t iree_hal_deferred_command_buffer_push_constants(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_PUSH_CONSTANTS,
      sizeof(*cmd) + sizeof(cmd->values[0]) * values_length, &cmd));
...
static iree_status_t iree_hal_deferred_command_buffer_push_descriptor_set(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_PUSH_DESCRIPTOR_SET,
      sizeof(*cmd) + sizeof(cmd->bindings[0]) * binding_count, &cmd));
  cmd->executable_layout = executable_layout;
  cmd->set = set;
...
static iree_status_t iree_hal_deferred_command_buffer_bind_descriptor_set(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_BIND_DESCRIPTOR_SET,
...
static iree_status_t iree_hal_deferred_command_buffer_dispatch(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_DISPATCH, sizeof(*cmd), &cmd));
...
static iree_status_t iree_hal_deferred_command_buffer_dispatch_indirect(
  ...
  IREE_RETURN_IF_ERROR(iree_hal_command_list_append_command(
      command_list, IREE_HAL_CMD_DISPATCH_INDIRECT, sizeof(*cmd), &cmd));
  cmd->executable = executable;
  cmd->entry_point = entry_point;
...
IREE_API_EXPORT iree_status_t iree_hal_deferred_command_buffer_apply(
  ...
  iree_hal_deferred_command_buffer_t* command_buffer =
      iree_hal_deferred_command_buffer_cast(base_command_buffer);
  iree_hal_command_list_t* command_list = &command_buffer->command_list;

  iree_status_t status = iree_hal_command_buffer_begin(target_command_buffer);
  if (iree_status_is_ok(status)) {
    for (iree_hal_cmd_header_t* cmd = command_list->head; cmd != NULL;
         cmd = cmd->next) {
